...
router.post('/',authMiddleware, async (req, res) => {
    try {
      const { deviceName,treeType, treeHeight, location,details,picture } = req.body;

      const userId = req.user._id;
      const user = await User.findById(userId);
      const newDevice = new Device({deviceName,treeType, treeHeight, location,details,picture ,user:user._id});
